...
import Function.ScoreExplanation;
...
public class IseDemo extends WebSocketListener {
    ...
    private static final String appid = "402cbb47";//控制台获取
    private static final String apiSecret = "MjVkMzc3MTU2ODQxNjJhZDQwZTg2MzA2";//控制台获取
    private static final String apiKey = "132828ac2b94045ac028643b5f272816";//控制台获取
    ...
    public String text;//评测试题,英文试题:[content]\nthere was a gentleman live near my house.
    IatSpeechView iatSpeechView;
    private static final String file = "iat_test.pcm";//评测音频,如传mp3格式请改变参数aue的值为lame
    ...
    public ArrayList<ArrayList<ArrayList<String>>> detail;
    public SyllableFrame syllableFrame;
    public DetailListener detailListener = new DetailListener();
    ...
    public IseDemo(IatSpeechView iatSpeechView) throws Exception {
        this.iatSpeechView = iatSpeechView;
        this.text = "[word]\n" + iatSpeechView.testWords.get(iatSpeechView.wordsIndex);
    ...
    private void ssb(WebSocket webSocket) {
        main.com.iflytek.IseDemo.ParamBuilder p = new main.com.iflytek.IseDemo.ParamBuilder();
        p.add("common", new main.com.iflytek.IseDemo.ParamBuilder()
                .add("app_id", appid)
        )
                .add("business", new main.com.iflytek.IseDemo.ParamBuilder()
                ...
                ).add("data", new main.com.iflytek.IseDemo.ParamBuilder()
                .add("status", 0)
                .add("data", ""));
    ...
    public void send(WebSocket webSocket, int aus, int status, String data) {
        main.com.iflytek.IseDemo.ParamBuilder p = new main.com.iflytek.IseDemo.ParamBuilder();
        p.add("business", new main.com.iflytek.IseDemo.ParamBuilder()
                .add("cmd", "auw")
                .add("aus", aus)
                .add("aue", "raw")
        ).add("data", new main.com.iflytek.IseDemo.ParamBuilder()
        ...
        if (resp != null) {
            ...
            if (resp.getData() != null) {
                ...
                if (resp.getData().getStatus() == 2) {
                    try {
                        iatSpeechView.correctLabel.setVisible(false);
                        iatSpeechView.wrongLabel.setVisible(false);
                        iatSpeechView.detail.setVisible(false);
                        String string = new String(decoder.decode(resp.getData().getData()), "UTF-8");
                        JSONObject jsonObject = xmlStrToJson(string);
                        if (ScoreExplanation.isCorrect(jsonObject)){
                            detail = ScoreExplanation.explainScore(jsonObject);
                            iatSpeechView.detail = new JButton("Detail");
                            iatSpeechView.detail.setBounds(237, 390, 100, 50);
                            iatSpeechView.add(iatSpeechView.detail);
                            iatSpeechView.detail.addActionListener(detailListener);
                            iatSpeechView.correctLabel.setVisible(true);
                            iatSpeechView.detail.setVisible(true);
                        }
                        else {
                            iatSpeechView.wrongLabel.setVisible(true);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    public JSONObject xmlStrToJson(String xmlString) throws JSONException {
        JSONObject parseJSON = null;
        try {
            parseJSON = XML.toJSONObject(xmlString).getJSONObject("xml_result"); //xmlStr传入的xml格式字符串，BaseLine xml中的顶层节点
            System.out.println(DictionaryLookup.prettify(parseJSON.toString()));
            System.out.println(ScoreExplanation.explainScore(parseJSON));
            System.out.println(ScoreExplanation.isCorrect(parseJSON));
