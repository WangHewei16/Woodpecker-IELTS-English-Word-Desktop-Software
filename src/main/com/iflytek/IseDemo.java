		...
		if (resp != null) {
			...
			if (resp.getData() != null) {
				...
				if (resp.getData().getStatus() == 2) {
					try {
						//System.out.println(new String(decoder.decode(resp.getData().getData()), "UTF-8"));
						//xmlStrToJson(new String(decoder.decode(resp.getData().getData()), "UTF-8"));
						new SyllableFrame(ScoreExplanation.explainScore(xmlStrToJson(new String(decoder.decode(resp.getData().getData()), "UTF-8"))));
					} catch (Exception e) {
						e.printStackTrace();
					}
